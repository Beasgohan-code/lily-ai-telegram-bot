...
class LilyCoreTests(unittest.TestCase):
    ...
    def test_live_activity_blocks_contain_only_public_status(self):
        ...
        # Full stage UI starts with heading; compact UI starts with thinking.
        self.assertTrue("thinking" in types or types[0] == "thinking")
        self.assertIn("Validating the selected file.", serialized)
        self.assertNotIn("chain-of-thought", serialized.lower())
        # Stages must appear as public checklist labels only.
        self.assertTrue(any("Validate file" in serialized or "Deliver result" in serialized for _ in [1]))
