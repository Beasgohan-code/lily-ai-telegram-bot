...
## Professional host control (CLI agent)

```bash
# Full guided install (dirs, .env from example, pip install)
python3 -m lily.cli deploy
# or
bash commands/deploy.sh

# Start / stop / restart / status / logs
python3 -m lily.cli host start
python3 -m lily.cli host start --api          # bot + FastAPI bridge
python3 -m lily.cli host status
python3 -m lily.cli host logs --tail 80
...
bash commands/host.sh guide                   # full git clone + VPS steps
```

Background processes write PIDs under `.lily_run/` and logs to `.lily_run/bot.log` / `.lily_run/api.log`.
