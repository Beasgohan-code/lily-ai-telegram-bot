...
from .db import db
from .rich import activity_status, blockquote, bold, buttons_block, code, confirmation_keyboard, custom_emoji, details, divider, heading, inline_keyboard, list_block, paragraph, preformatted, rich, rich_message_button, rich_text_button, table
...
from .agent_team import public_team_stages, public_team_summary
from .errors import public_error_message
from .group_ops import GroupOpsError, ban_member, format_duration, guard_moderation_target, kick_member, mute_member, resolve_member_label, set_description, set_title, unban_member, unmute_member
...
def _ensure_staged_session(
    context: ContextTypes.DEFAULT_TYPE,
    update: Update,
    *,
    stages: list[str] | None = None,
    summary: str = "Processing your request with Lily agents",
) -> "LiveThinkingSession":
    """Return the active live session or create one with progressive stage UI."""
    session = active_session(context)
    if session is not None:
        if stages:
            session.stages = [str(s)[:180] for s in stages][:8]
...
async def _finish_session(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Safely clear any active thinking draft."""
    session = active_session(context)
    if session:
...
async def progress_message(update: Update, text_value: str, *, context: ContextTypes.DEFAULT_TYPE | None = None) -> None:
    """Update the live staged draft in place — never spam chat with working messages."""
    chat = update.effective_chat
    if not chat:
        return
    ...
    session = active_session(context) if context else None
    if session:
        # Prefer advance when the session already tracks stages; otherwise simple update.
        if getattr(session, "stages", None) and session.is_active and hasattr(session, "update"):
...
def _auto_poll_options(question: str) -> list[str]:
    """Lightweight AI-style option suggestions when the user only gives a question.

    Deterministic heuristics keep this offline-safe; when the AI client is
    available the planner can still override via plan.args.options.
    """
    q = (question or "").strip().lower()
    if not q:
        return ["Yes", "No", "Maybe"]
    if any(w in q for w in ("yes or no", "yes/no", "should we", "do we", "can we", "?")):
        base = ["Yes", "No", "Not sure"]
    elif any(w in q for w in ("when", "time", "day", "date")):
        base = ["Today", "Tomorrow", "This weekend", "Next week"]
    elif any(w in q for w in ("where", "place", "location")):
        base = ["Option A", "Option B", "Somewhere else"]
    elif any(w in q for w in ("who", "which", "prefer", "best", "favorite")):
        base = ["Choice 1", "Choice 2", "Choice 3", "Other"]
    else:
        base = ["Agree", "Disagree", "Need more info", "Abstain"]
    # Personalize lightly from keywords in the question
    words = [w for w in re.findall(r"[a-z0-9]{3,}", q) if w not in {"the", "and", "for", "poll", "what", "how", "when", "where", "which", "with", "this", "that", "from", "have", "will"}]
    if len(words) >= 2:
        base = [words[0].title(), words[1].title()] + base[:2]
...
async def execute_plan(update: Update, context: ContextTypes.DEFAULT_TYPE, plan: Plan) -> str:
    ...
    if action == "ban_user":
        ...
        await guard_moderation_target(bot, chat_id, user_id, target, bot_id=bot.id)
        label = await resolve_member_label(bot, chat_id, target)
        revoke = bool(plan.args.get("revoke_messages", True))
        until = None
        if plan.args.get("seconds"):
            secs = max(60, min(int(plan.args["seconds"]), 2_419_200))
            until = datetime.now(timezone.utc) + timedelta(seconds=secs)
        await ban_member(bot, chat_id, target, revoke_messages=revoke, until=until)
        await db.audit(chat_id, user_id, "ban_user", {**plan.args, "label": label})
        extra = f" until {until.isoformat()}" if until else ""
        return f"Banned {label}{extra}."
    if action == "unban_user":
        bot = update.get_bot()
        target = int(plan.args["user_id"])
        label = await resolve_member_label(bot, chat_id, target)
        await unban_member(bot, chat_id, target)
        await db.audit(chat_id, user_id, "unban_user", {**plan.args, "label": label})
        return f"Unbanned {label}. They can rejoin with an invite link."
    if action == "demote_user":
        ...
        label = await resolve_member_label(bot, chat_id, target)
        try:
            await bot.promote_chat_member(chat_id, target, is_anonymous=False, can_manage_chat=False, can_delete_messages=False, can_manage_video_chats=False, can_restrict_members=False, can_promote_members=False, can_change_info=False, can_invite_users=False, can_pin_messages=False)
        except Exception as exc:
            raise GroupOpsError(str(exc)[:300]) from None
        await db.audit(chat_id, user_id, "demote_user", plan.args)
        return f"Demoted {label}."
    if action == "promote_user":
        ...
        label = await resolve_member_label(bot, chat_id, target)
        try:
            await bot.promote_chat_member(chat_id, target, can_manage_chat=True, can_delete_messages=True, can_manage_video_chats=True, can_restrict_members=True, can_promote_members=False, can_change_info=True, can_invite_users=True, can_pin_messages=True)
        except Exception as exc:
            raise GroupOpsError(str(exc)[:300]) from None
        await db.audit(chat_id, user_id, "promote_user", plan.args)
        return f"Promoted {label} with safe moderator permissions."
    if action == "kick_user":
        bot = update.get_bot()
        target = int(plan.args["user_id"])
        await guard_moderation_target(bot, chat_id, user_id, target, bot_id=bot.id)
        label = await resolve_member_label(bot, chat_id, target)
        await kick_member(bot, chat_id, target)
        await db.audit(chat_id, user_id, "kick_user", {**plan.args, "label": label})
        return f"Removed {label} from the group."
    if action == "mute_user":
        bot = update.get_bot()
        target = int(plan.args["user_id"])
        await guard_moderation_target(bot, chat_id, user_id, target, bot_id=bot.id)
        ...
        until = await mute_member(bot, chat_id, target, seconds)
        await db.audit(chat_id, user_id, "mute_user", {**plan.args, "label": label, "until": until.isoformat()})
        return f"Muted {label} for {format_duration(seconds)} (until {until.strftime('%Y-%m-%d %H:%M UTC')})."
    if action == "unmute_user":
        bot = update.get_bot()
        target = int(plan.args["user_id"])
        label = await resolve_member_label(bot, chat_id, target)
        await unmute_member(bot, chat_id, target, normal_chat_permissions())
        await db.audit(chat_id, user_id, "unmute_user", {**plan.args, "label": label})
        return f"Unmuted {label}."
    ...
    if action == "set_chat_title":
        title = await set_title(update.get_bot(), chat_id, str(plan.args.get("title") or ""))
    ...
    if action == "set_chat_description":
        ...
        await set_description(update.get_bot(), chat_id, description)
    ...
    if action == "create_invite_link":
        ...
        join_request = bool(plan.args.get("creates_join_request") or plan.args.get("join_request"))
        ...
        if expire_hours:
            payload["expire_date"] = int(datetime.now(timezone.utc).timestamp()) + int(expire_hours) * 3600
        if join_request:
        ...
        if not link:
            raise GroupOpsError("Telegram did not return an invite link.")
        ...
        bits = [f"Invite link: {link}"]
        if name:
            bits.append(f"Name: {name}")
        if member_limit:
            bits.append(f"Member limit: {member_limit}")
        if expire_hours:
            bits.append(f"Expires in: {expire_hours}h")
        if join_request:
            bits.append("Join requests enabled")
    ...
    if action == "create_poll":
        ...
        auto = bool(plan.args.get("auto_options"))
        if not question:
            return "A poll needs a question."
        if len(options) < 2:
            if auto or not options:
                options = _auto_poll_options(question)
            else:
                return "A poll needs a question and at least two options."
        # Rich preview with RichTextButton entities + buttons block (final message — enabled)
        option_lines = []
        for index, opt in enumerate(options, start=1):
            option_lines.append(
                paragraph([
                    f"{index}. ",
                    rich_text_button(opt[:40], callback_data=f"pollopt:{index-1}", style="primary"),
                ])
            )
        preview = [
            heading("Poll ready", 3),
            paragraph(question),
            *option_lines,
            buttons_block([
                [rich_message_button("Post poll", callback_data="poll:post", style="success")],
            ]),
        ]
        # Always post the real Telegram poll (native voting UX); rich buttons are a visual summary.
        await update.get_bot().send_poll(
            chat_id,
            question,
            options,
            is_anonymous=bool(plan.args.get("anonymous", True)),
            type="quiz" if plan.args.get("quiz") else "regular",
        )
        try:
            await rich.send(chat_id, preview)
        except Exception:
            pass
        await db.audit(chat_id, user_id, "create_poll", {
            "question": question,
            "option_count": len(options),
            "auto_options": auto or len(plan.args.get("options") or []) < 2,
        })
        return f"Poll posted with {len(options)} option(s)" + (" (AI-suggested)." if auto or len(plan.args.get("options") or []) < 2 else ".")
    ...
    if action == "publish_channel_post":
        ...
        bot = update.get_bot()
        ok, label = await post_service.verify_channel(bot, channel_id, user_id)
        if not ok:
        ...
        result = await rich.send(target_chat, blocks, protect_content=False)
        message_id = result.get("message_id") if isinstance(result, dict) else None
        if not message_id and isinstance(result, dict):
            # Some Bot API wrappers nest the message
            message_id = (result.get("result") or {}).get("message_id") if isinstance(result.get("result"), dict) else None
        pin_requested = bool(plan.args.get("pin") or plan.args.get("pin_message"))
        pinned = False
        if message_id and pin_requested:
            try:
                await bot.pin_chat_message(target_chat, int(message_id), disable_notification=True)
                pinned = True
            except Exception:
                pinned = False
        if message_id:
        ...
        link = ""
        if message_id and raw_channel.startswith("-100"):
            link = f"https://t.me/c/{raw_channel[4:]}/{message_id}"
        elif message_id and not str(channel_id).lstrip("-").isdigit():
            link = f"https://t.me/{str(channel_id).lstrip('@')}/{message_id}"
        await db.audit(chat_id, user_id, "publish_channel_post", {"channel_id": str(channel_id), "message_id": message_id, "pinned": pinned})
        bits = [f"Published to **{label}**."]
        if message_id:
            bits.append(f"Message ID: `{message_id}`")
        if link:
            bits.append(f"Link: {link}")
        if pin_requested:
            bits.append("Pinned." if pinned else "Pin requested but Lily could not pin (needs pin rights).")
        return "\n".join(bits)
    ...
    if action in {"rename_file", "compress_file", "encode_media", "create_file", "download_song"}:
        ...
        if action == "rename_file":
            ...
            return f"File renamed and sent as `{path.name}`. Reply to another file and ask Lily to rename it again anytime."
...
async def handle_plan(update: Update, context: ContextTypes.DEFAULT_TYPE, plan: Plan, chat_settings: dict[str, Any]) -> None:
    ...
    # Ensure every plan path has a live staged session with real role/plan stages
    session = _ensure_staged_session(
        context,
        update,
        stages=public_stages[:8] if public_stages else None,
        summary=(plan.summary or "Working on your request")[:200],
    )
    if public_stages:
        session.stages = [str(s)[:180] for s in public_stages][:8]
        session.summary = (plan.summary or session.summary)[:200]
    if not session.is_active:
        await session.start_plan("Reviewing plan…")
    else:
        await session.advance("Safety review…")
    ...
    if plan.action == "none":
        ...
        if hasattr(session, "start_work"):
            await session.start_work("Composing a response…")
        else:
        ...
        if hasattr(session, "advance"):
            await session.advance("Reply ready")
        await session.finish()
    ...
    if plan.requires_confirmation or plan.risk in {"risky", "dangerous"}:
        ...
        chat = update.effective_chat
        await rich.send_confirmation(
            chat.id,
            confirm_blocks,
            confirmation_keyboard(action_id),
            requester_id=update.effective_user.id,
            chat_type=getattr(chat, "type", None),
            reply_to=update.effective_message.message_id if update.effective_message else None,
        )
        return
    session = active_session(context) or bind_session(
        context, update,
        stages=public_stages[:8] if public_stages else None,
        summary=plan.summary[:200] if plan.summary else "Working on your request",
    )
    if hasattr(session, "start_work"):
        await session.start_work("Executing plan…")
    else:
        await session.update("Working…")
    try:
        result = await execute_plan(update, context, plan)
    except GroupOpsError as exc:
        await session.finish()
        await _finish_skill_plan(plan, "failed", "Group action rejected")
        await send_error(update, str(exc)[:500])
        return
    except Exception:
        await session.finish()
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if plugin_plan:
        session = _ensure_staged_session(
            context, update,
            stages=["Match plugin", "Build plan", "Safety review", "Execute or confirm", "Reply"],
            summary="Running a Lily plugin skill",
        )
        await session.start_plan("Matching plugin…")
        await session.advance("Building plugin plan…")
        await handle_plan(update, context, plugin_plan, settings_for_chat)
        return
    if skill_plan:
        session = _ensure_staged_session(
            context, update,
            stages=["Match skill", "Build plan", "Safety review", "Execute or confirm", "Reply"],
            summary="Running a Lily custom skill",
        )
        await session.start_plan("Matching skill…")
        await session.advance("Building skill plan…")
        await handle_plan(update, context, skill_plan, settings_for_chat)
        return
    memories = await db.recent_memories(f"chat:{update.effective_chat.id}:user:{update.effective_user.id}")
    # Real progressive UI: plan stages from the agent catalog, refined after team_plan
    default_stages = [
        "Understand request",
        "Assign specialist roles",
        "Build action plan",
    ...
    session = bind_session(
        ...
        summary="Processing your request with Lily agents",
    )
    await session.start_plan("Understanding your request…")
    await session.advance("Assigning specialist roles…")
    plan = await ai.team_plan(text_value, _reply_context(update), memories, settings_for_chat)
    # Advanced agent swarm: specialists drive the live stage UI
    try:
        plan, swarm = enrich_plan_with_swarm(plan, text_value)
        session.stages = swarm.stages() or session.stages
        session.summary = f"{swarm.lead} · {len(swarm.members)} specialists"
        session.active_index = 0
        # Push a rich multi-agent thinking draft
        try:
            await rich.draft(
                session.chat_id,
                swarm_live_blocks(swarm, status="Specialists reviewing…", phase="plan"),
                draft_id=session.draft_id,
            )
        except Exception:
            pass
        context.user_data["_agent_swarm"] = swarm
    except Exception:
        try:
            roles = assign_roles(plan)
            team_stages = public_team_stages(plan)
            plan_stages = plan.public_stages() if hasattr(plan, "public_stages") else []
            enriched = [*roles.public_stages()[:3], *team_stages[:2], *plan_stages[:3]]
            if enriched:
                session.stages = [str(s)[:180] for s in enriched][:8]
        except Exception:
            pass
    await session.advance("Plan ready — reviewing safety…")
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data in {"postpublish", "postcancel"}:
        ...
        await rich.send_confirmation(
            chat.id,
            [
                heading("Final publish confirmation", 2),
                paragraph(f"Publish this draft to {state['channel_label']}?"),
                paragraph("Lily must be an administrator in that channel with permission to post."),
                blockquote("Publishing will create a real channel post.", "Lily"),
            ],
            confirmation_keyboard(action_id),
            requester_id=update.effective_user.id,
            chat_type=getattr(chat, "type", None),
    ...
    await query.edit_message_reply_markup(reply_markup=None)
    # Restart staged UI for the approved execution path
    roles = assign_roles(plan)
    stages = [*roles.public_stages()[:3], *plan.public_stages()[:4], "Reply"]
    session = _ensure_staged_session(
        context, update,
        stages=stages[:8],
        summary=(plan.summary or "Executing approved action")[:200],
    )
    await session.start_work("Approval received — executing…")
    try:
        if plan.action == "encode_media":
            job_id = await encoding_queue.enqueue(update, context, plan, execute_plan)
            await session.advance("Queued background job")
            await session.finish()
            ...
            return
        await session.advance("Running action…")
        result = await execute_plan(update, context, plan)
        await session.advance("Done")
        await session.finish()
        await send_result(update, result, context=context)
...
async def on_stopped_generation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if session:
        await session.finish()
    if settings.draft_keep_on_stop:
        note = "Generation stopped. The partial response was kept as shown above."
    else:
        note = "Generation stopped. The partial draft was discarded."
    await rich.send(
        chat.id,
        [heading("Stopped", 3), paragraph(note)],
        reply_to=update.effective_message.message_id if update.effective_message else None,
    )
