...
"""
    ...
    def heuristic_plan(self, text: str, context: dict[str, Any]) -> Plan:
        ...
        if any(word in low for word in ("create poll", "make a poll", "start a poll", "auto poll", "ai poll")):
            body = re.sub(r"^.*?(?:create|make|start|auto|ai)\s+(?:a\s+)?poll\s*:?\s*", "", value, flags=re.I).strip()
            parts = [part.strip() for part in body.split("|") if part.strip()]
            question, options = (parts[0], parts[1:]) if parts else (body, [])
            auto_options = not options or "auto" in low or "ai poll" in low or "auto poll" in low
            if question and not options:
                auto_options = True
            missing = [] if question and (auto_options or 2 <= len(options) <= 10) else ["Use: create poll: Question | Option 1 | Option 2  (or just the question for AI options)"]
            return Plan(
                intent="create_poll",
                summary="Create a group poll" + (" with AI options" if auto_options else ""),
                action="create_poll",
                risk="risky",
                ...
                args={
                    "question": question,
                    "options": options,
                    "anonymous": "non-anonymous" not in low,
