...
class Settings:
    ...
    enable_agent_team: bool = field(default_factory=lambda: _bool("LILY_ENABLE_AGENT_TEAM", True))
    ...
    rich_visible_progress: bool = field(default_factory=lambda: _bool("LILY_RICH_VISIBLE_PROGRESS", False))
    rich_button_styles: bool = field(default_factory=lambda: _bool("LILY_RICH_BUTTON_STYLES", True))
    enable_message_drafts: bool = field(default_factory=lambda: _bool("LILY_ENABLE_MESSAGE_DRAFTS", True))
    enable_ai_thinking_indicator: bool = field(default_factory=lambda: _bool("LILY_ENABLE_AI_THINKING", True))
    enable_ephemeral_confirmations: bool = field(default_factory=lambda: _bool("LILY_EPHEMERAL_CONFIRMATIONS", True))
    draft_can_stop: bool = field(default_factory=lambda: _bool("LILY_DRAFT_CAN_STOP", True))
    draft_keep_on_stop: bool = field(default_factory=lambda: _bool("LILY_DRAFT_KEEP_ON_STOP", True))

    tool_retry_count: int = field(default_factory=lambda: max(0, min(_int("LILY_TOOL_RETRY_COUNT", 2), 5)))
    tool_subprocess_timeout: int = field(default_factory=lambda: max(30, min(_int("LILY_TOOL_SUBPROCESS_TIMEOUT", 600), 3600)))
    tool_download_timeout: int = field(default_factory=lambda: max(30, min(_int("LILY_TOOL_DOWNLOAD_TIMEOUT", 120), 600)))

    compact_responses: bool = field(default_factory=lambda: _bool("LILY_COMPACT_RESPONSES", False))
    ...
    enable_free_tools: bool = field(default_factory=lambda: _bool("LILY_ENABLE_FREE_TOOLS", True))
    free_api_timeout: float = field(default_factory=lambda: max(5.0, min(float(os.getenv("LILY_FREE_API_TIMEOUT", "20")), 60.0)))
    free_api_user_agent: str = field(default_factory=lambda: os.getenv("LILY_FREE_API_USER_AGENT", "LilyTelegramBot/2.0 (+https://braindaemon.com)"))
